...
if (isset($_POST['clave_nombre'])){
	$clave_nombre = $_POST['clave_nombre'];
	 BuscarArticuloMicrosip($clave_nombre);
...
FROM ARTICULOS ARTI
	...
	LEFT JOIN NIVELES_ARTICULOS NIVA ON NIVA.ARTICULO_ID = ARTI.ARTICULO_ID AND NIVA.ALMACEN_ID = '$id_almacen_microsip'
	
WHERE (CLAVES.ROL_CLAVE_ART_ID = '17')
AND (ARTI.ESTATUS = 'A')
AND (ARTI.ES_ALMACENABLE = 'S')
AND (ARTI.NOMBRE LIKE '%$clave_nombre%')
AND (ARTI.ARTICULO_ID IN(SELECT DOCS_INDET.ARTICULO_ID FROM DOCTOS_IN_DET DOCS_INDET WHERE (DOCS_INDET.ALMACEN_ID = '$id_almacen_microsip')))";
...
while ($row=$res->fetch()){
			$count++;
			$existencia = ExistenciaMicrosip($row->ARTID);
		...
		$tabla .= '<tr><td>'.$row->CLAVE.'</td>
					...
					<td align="center">
					<input type="button" class="btn btn-success btn-lg add_articulo_microsip" value="+" id="btnid_'.$row->ARTID.'">
					
					<input type="hidden"  value="'.$row->CLAVE.'" id="tx_clave_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->ARTICULO.'" id="tx_articulo_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->UDM.'" id="tx_udm_'.$row->ARTID.'">
					<input type="hidden"  value="'.$existencia.'" id="tx_existencia_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->PRECIO.'" id="tx_precio_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->MAXIMO.'" id="tx_maximo_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->MINIMO.'" id="tx_minimo_'.$row->ARTID.'">
					<input type="hidden"  value="'.$row->REORDEN.'" id="tx_reorden_'.$row->ARTID.'">
...
<script>
	$(document).ready(function(){
				...
				$(".add_articulo_microsip").on("click", function(){
							...
							var empresa = document.getElementById("select_art_microsip_empresa").value;
