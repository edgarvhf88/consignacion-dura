...
$consulta_relacion = "
SELECT p.fecha_pedido as fecha_pedido, p.id as id_pedido, p.folio as folio, p.total_pedido as total_pedido, p.estatus as estatus, p.id_usuario as id_requi, p.id_empresa as id_empresa, p.orden_compra as orden_compra, p.tracking as tracking, s.sucursal as sucursal
FROM supervisor_relacion r
LEFT JOIN pedidos p on r.id_subordinado = p.id_usuario
INNER JOIN sucursales s on s.id_sucursal = p.id_dir_suc
WHERE r.id_supervisor = '$id_usuario' and p.estatus <> '0' and p.estatus <> '0p' and p.id_empresa = '$id_empresa'
ORDER BY folio DESC";
...
echo '<table class="table table-striped table-bordered table-hover table-responsive">
                    	<tr class="info">
                            ...
                            <th>'.$sucursal_tabla_mis_pedidos.'</th>
...
else	
{
 ...
 echo ' <tr onclick="detalle_pedido('.$row2['id_pedido'].','.$row2['folio'].','.$row2['total_pedido'].');">
                    		
                   	<td >'.Nombre($row2['id_requi']).'</td>
                   	
                   	<td align="right" >'.$row2['folio'].'</td>
					<td align="right" >'.$row2['fecha_pedido'].'</td>
					<td align="right" >'.$row2['orden_compra'].'</td>
                   	<td align="right" >$'.number_format($row2['total_pedido'],2).'</td>
                   	<td align="center" id="td_estatus_'.$row2['id_pedido'].'" '.$clase_td.'>'.$estatus.'</td>
					<td align="center" >
							<input type="hidden" id="txt_folio_pedido_'.$row2['id_pedido'].'" value="'.$row2['folio'].'"/>
							'.$tracking.'
					</td>
					<td>'.$row2['sucursal'].'</td>					
                    </tr>
